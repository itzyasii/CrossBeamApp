...
export function AnalyticsScreen() {
  ...
  const rate =
    data.totalFiles + data.totalFailed > 0
      ? Math.round(
          (data.totalFiles / (data.totalFiles + data.totalFailed)) * 100,
        )
      : 100;
    ...
    >
      ...
      <GlassCard animate accentBorder>
        <View style={S.heroMetric}>
          <View>
            ...
            <View style={S.heroValueRow}>
              ...
              <Text style={[S.heroUnit, { color: colors.textSecondary }]}>
                {heroParts[1] ?? "B"}
              </Text>
            </View>
          </View>
        ...
        <Text
          style={[
            S.trendText,
            { color: peakDayBytes > 0 ? colors.success : colors.textMuted },
          ]}
        >
          {peakDayBytes > 0
            ? `${formatBytes(peakDayBytes)} moved on the busiest day this week`
            : "No completed transfer data yet"}
        </Text>
      </GlassCard>
      ...
      <GlassCard>
        <View style={S.efficiencyRow}>
          <View style={{ flex: 1 }}>
            <Text style={[S.efficiencyLabel, { color: colors.textSecondary }]}>
              Success Rate
            </Text>
            <View style={S.barTrack}>
              <LinearGradient
                ...
                style={[S.barFill, { width: `${rate}%` }]}
              />
            </View>
          </View>
          <Text style={[S.efficiencyVal, { color: colors.accent }]}>
            {rate}%
          </Text>
        </View>
        ...
        <View style={S.summaryRow}>
          <View style={S.summaryItem}>
            <Text style={[S.summaryVal, { color: colors.textPrimary }]}>
              {data.totalJobs}
            </Text>
        ...
        <View style={S.chart}>
          {data.dailyBytes.map((bytes, index) => {
            ...
            const height = Math.max(
              4,
              Math.round((bytes / maxDailyBytes) * 100),
            );
            return (
      ...
      <View style={S.logSection}>
        ...
        <GlassCard padding={0}>
          ...
          {data.recentTransfers.map((transfer, index) => (
            ...
            >
              <View
                style={[S.logIcon, { backgroundColor: colors.surfaceHover }]}
              >
                <FileText
                  size={20}
