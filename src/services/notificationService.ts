...
export const notificationService = {
  ...
  showIncomingTransferRequest: async (approval: IncomingApprovalRequest) => {
    ...
    if (Platform.OS === "android") {
      try {
        ...
        console.warn(
          "[Notification] native showIncomingNotification failed:",
          e,
        );
      }
    }
    ...
    await Notifications.scheduleNotificationAsync({
      ...
      content: {
        ...
        ...(Platform.OS === "android" ? { channelId: INCOMING_CHANNEL } : {}),
      },
      trigger: null,
    });
  },
  ...
  dismissIncomingTransferRequest: async (transferId: string) => {
    ...
    if (Platform.OS === "android") {
      try {
        await nativeCrossBeam.dismissIncomingNotification(transferId);
        return;
      } catch (e) {
        console.warn(
          "[Notification] native dismissIncomingNotification failed:",
          e,
        );
      }
    }
